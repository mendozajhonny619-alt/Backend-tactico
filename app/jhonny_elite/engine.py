...
class JhonnyEliteEngine:
    ...
    def _economy_priority(self, match: Dict[str, Any]) -> float:
        ...
        under_hint = sf(match.get("under_probability"), 0.0) if minute >= int(getattr(Config, "UNDER_PREP_MINUTE", 68)) else 0.0
    ...
    def analyze_match(self, match: Dict[str, Any]) -> Dict[str, Any]:
        ...
        candidate_floor = (
            sf(getattr(Config, "UNDER_CANDIDATE_MIN_CONFIDENCE", 66), 66)
            if direction == "UNDER"
            else sf(getattr(Config, "CANDIDATE_PREMATCH_MIN_CONFIDENCE", 72), 72)
        )
        candidate = (
        ...
        if enrichment_allowed:
            ...
            odds_floor = (
                sf(getattr(Config, "UNDER_ODDS_PREFETCH_MIN_SCORE", 66), 66)
                if direction == "UNDER"
                else sf(getattr(Config, "ODDS_MIN_CANDIDATE_SCORE", 76), 76)
            )
            if candidate_score >= odds_floor:
    ...
    def _live_candidate_scores(
        ...
        if minute < under_prep:
            under = min(under, 45.0)
        elif minute < under_target:
            # Preparar el candidato antes del 75 para que prepartido y cuota esten listos.
            under += min(6.0, max(0.0, minute - under_prep) * 0.9)
        elif minute < under_late:
            # Ventana objetivo: si el partido esta realmente cerrado, el score debe
            # madurar aqui, no recien en 80-85.
            under += 5.0
        else:
            # Si no fue suficientemente claro cerca de 75, no premiar artificialmente
            # el simple paso del tiempo. Entradas muy tardias se vuelven mas estrictas.
            under -= min(10.0, max(0.0, minute - under_late) * 2.0)
    ...
    def _candidate_direction(self, scores: Dict[str, float], minute: int) -> tuple[Optional[str], float]:
        ...
        under_prep_minute = int(getattr(Config, "UNDER_PREP_MINUTE", 68))

        if minute < under_prep_minute:
    ...
    def _critical_blockers(
        ...
        for value in risk.get("hard_blockers", []) or []:
            if value and value not in blockers:
                blockers.append(str(value))
        # UNDER puede ser candidato antes de su ventana de entrada para adelantar
        # prepartido/cuota. MasterDecisionAI es quien impide publicarlo demasiado pronto.
        return blockers
